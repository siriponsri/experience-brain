...
| EXP-03.2 | Knowledge Inbox and Dual-Memory Foundation | v0.2.3 | completed | Adds append-only `knowledge.jsonl`, inbox processing, source extractors, Knowledge dashboard views, and separated Knowledge/Experience retrieval | Owner-live inbox validation passed on `a862462` and is merged into `main`; source provenance preserved; no Experience auto-created; CLI/MCP unified retrieval separated Knowledge and Experience; tests, Ruff, format, mypy, and lint pass | Better low-code source ingestion foundation; not benchmark evidence | Ready for v0.3.0-poc integration freeze after final hygiene checks |
